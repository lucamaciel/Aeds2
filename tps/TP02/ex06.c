...
Restaurante *parse_restaurante(char *s){
    ...
    prox_campo(s,&pos,',',r->nome,MAX_STR);
    prox_campo(s,&pos,',',r->cidade,MAX_STR);
...
void formatar_restaurante(Restaurante *r,char *buffer){
    ...
    sprintf(buffer,"[%d ## %s ## %s ## %d ## %.1f ## %s ## %s ## %s-%s ## %s ## %s]",
        ...
        tipos,fp,ba,bf,bd,r->aberto?"true":"false");
}
...
void ler_csv_colecao(Colecao_Restaurantes *col,char *path){
    FILE *f=fopen(path,"r");
    if(!f){fprintf(stderr,"Nao encontrado: %s\n",path);return;}
    int cap=100;
    col->restaurantes=(Restaurante**)malloc(cap*sizeof(Restaurante*)); col->tamanho=0;
...
// ============================================================
// Selection Sort por nome (reutilizado do ex03)
// ============================================================
static long long comp_ord = 0, mov_ord = 0;

void selection_sort(Restaurante **v,int n){
    for(int i=0;i<n-1;i++){
        int min=i;
        for(int j=i+1;j<n;j++){comp_ord++;if(strcmp(v[j]->nome,v[min]->nome)<0)min=j;}
        if(min!=i){Restaurante *t=v[i];v[i]=v[min];v[min]=t;mov_ord++;}
    }
}
...
int pesquisa_binaria(Restaurante **v,int n,const char *nome){
    int esq=0,dir=n-1;
    while(esq<=dir){
        int meio=(esq+dir)/2;
        comparacoes++;
...
int main(){
    ...
    // Parte 1: ids
    int ids[10000]; int n_ids=0, id;
    while(scanf("%d",&id)==1){if(id==-1)break;ids[n_ids++]=id;}
    // consome \n
    char lixo[8]; fgets(lixo,sizeof(lixo),stdin);

    Restaurante **base=(Restaurante**)malloc(n_ids*sizeof(Restaurante*));
    int n=0;
    for(int i=0;i<n_ids;i++){
        for(int j=0;j<colecao->tamanho;j++)
            if(colecao->restaurantes[j]->id==ids[i]){base[n++]=colecao->restaurantes[j];break;}
    }

    // ordena por nome (selection sort)
    ...
    char nome_busca[MAX_STR];
    while(fgets(nome_busca,sizeof(nome_busca),stdin)){
        int len=my_len(nome_busca);
        while(len>0&&(nome_busca[len-1]=='\n'||nome_busca[len-1]=='\r'))nome_busca[--len]='\0';
        if(strcmp(nome_busca,"FIM")==0) break;
        int idx=pesquisa_binaria(base,n,nome_busca);
        printf("%s\n",idx>=0?"SIM":"NAO");
    }
    ...
    FILE *log=fopen("888234_binaria.txt","w");
    if(log){fprintf(log,"888234\t%lld\t%.6f\n",comparacoes,tempo);fclose(log);}
